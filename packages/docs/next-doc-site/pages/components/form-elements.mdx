...
- **Switch** - Allow the user to turn an individual option on or off. Should be used when the instant response of applied settings is required without additional action.
...
`Switch` accepts all props regular HTML checkbox would accept, such as `disabled`, `checked`, `value` or `onChange`.
