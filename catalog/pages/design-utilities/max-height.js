  ...
  return (
    <main>
      ...
      <Page>
        <header>
          <h2>Additive</h2>
          <p>These classes add <code>max-height</code> properties to an element, so they can be used for building or customizing an element.</p>
        </header>

        <h4 className="dc-u-mb-0 dc-u-mt-32 dc-u-minw-100pc">Pixels</h4>
        <p>
          Max Height utilites can be used with pixels using the format <code>.dc-u-maxh-[measure]</code>, where <code>[measure]</code> is one of the following: <code>0, {pixels.join(', ')}</code>
        </p>
        ...
        <p>Max Height utilites can also be used with the following percentage units and auto.</p>
